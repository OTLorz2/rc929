...
| **单一交付物** | 只产出一份自包含 `.html`（CSS/JS 内联；字体/Mermaid 可用 CDN）。Agent 先写 `.md`，再通过内置脚本转换。 |
...
**需要 Node.js 18+** 用于 Markdown 转 HTML（无需 `npm install`）。
...
- **证据面板** —— 可折叠代码片段，支持语法高亮（`language-*` class）；路径可回溯到仓库。
...
```
 intake → 规划（2–6 个方向）→ 定位（WHERE）→ 分析（HOW）→ 交叉验证 → 写 Markdown → 转 HTML → 交付
```
...
6. **写 Markdown** —— 按 `references/markdown-report-guide.md` 撰写 `<YYYY-MM-DD>-research-<topic>.md`。
7. **转换** —— 运行 `node .claude/skills/rc929/scripts/md-to-html.mjs <path.md>` 生成 HTML 报告。
8. **交付** —— 给出 HTML 路径、2–3 句事实摘要、建议先看哪张图/哪一节、以及未闭合的缺口。
...
- [markdown-report-guide.md](.claude/skills/rc929/references/markdown-report-guide.md) —— Agent 撰写报告的 Markdown 规范
